...
output:
  html_document:
    ...
    fig_width: 9
    fig_height: 7
abstract: "The St. Louis Blues won the National Hockey League's Stanley Cup at the end of the 2018-19 season, crowning them champions for the first time in their NHL history. The below paper examines the change in St. Louis Blues' performance throughout the season by simulating the remainder of the season and finals playoffs from two demarcations in the data and using the results from the entire season. \nConstant Elo ratings were calculated for each partition in the data set and Monte Carlo models were built around these Elo ratings to simulate all games occurring after the partition. The simulations highlighted the extent of the turn around in the season for the St. Louis Blues, but still showed that their chances of winning the Stanley Cup at the end of the regular season were comparatively slim. This study could have been improved by changing the method used to partition the data, by changing how season standings were calculated, and providing further granularity to the simulations."
---
...
The St. Louis Blues were the winners of the 2018-2019 Stanley Cup playoffs, winning a 4-3 series against the Boston Bruins. The Blues winning the Stanley Cup was paticularly extraordinary not only because it was the first in their history as a club in the NHL, but also because they faced off against the Bruins who have appeared so regularly in the Stanley Cup Playoffs. Since joining the NHL in 1967, the St Louis Blues have contested the Stanley Cup Playoffs only 4 times, whilst the Boston Bruins have made it to the Cup Playoffs 10 times since 1967 (and a total 20 since joining the NHL in 1924). (NHL, 2019)  
...
The NHL contained a total of 31 teams in the 2019 season. These 31 teams were split across two conferences, Eastern and Western. The conferences comprised two divisions each; Metropolitan and Atlantic in the Eastern Conference and Central and Pacific in the Western Conference. (NHL, 2019)    

Throughout the regular season each team played 82 games, which were a mixture of home and away fixtures. As the playing season was approximately 27 weeks, these 82 games averaged out to be more than three games per week. Therefore, unlike many Australian sporting codes, there were no "rounds" in the NHL but simply game numbers. Further, there is no structure or restriction to which game each team is playing. For example, one team may be playing their 26th game against an opponent that is playing their 23rd. (NHL, 2019)  
If a team won a game in the regular season they were awarded 2 points for the win. If the game had a tied scoreline at the end of the third 20-minute period it went into overtime. If the scoreline was still tied at the end of two overtime periods then a shootout was played until a winner was decided. The team that won by any method, including overtime or shootout, was awarded 2 points towards their season standings. A team that lost in overtime or shootout was awarded one point. If the game was decided before overtime the losing team was awarded 0 points. (MacDonald, R, 2018)
...
Once the regular season finished and team standings had been calculated, a bracketed finals system was drawn up for the playoffs. The finals playoffs followed a repeating knockout structure that has 3 rounds before reaching the Stanley Cup playoffs. Each conference follows the same structure, so only one will be described as follows:  
...
Considering the turn around in the St. Louis Blues' performance within the season and the lengthy multiple finals series, what were the chances that the St. Louis Blues could win the Stanley Cup from their standings at the end of the regular season? Further, did this chance change considerably from their standings at the end of their 25th and 50th games?
...
Regular season game schedule and results were taken from [Hockey-Reference.com](https://www.hockey-reference.com/leagues/NHL_2019_games.html), a subsidiary of Sports Reference LLC. The data contained Date of Game, Visiting (Away) Team, Goals scored by Away Team, Home Team, Goals scored by Home Team, Win Type (Regular, OT, SO), Attendance, and Length of Game for all games by all teams in the regular season. All of the above variables were used in the following analysis, with Attendance and Length of Game not being considered.
...
3. No partition, where teams had played the full 82 games of the season
...
The above points system using overtime losses was not used in each stage of the finals playoffs as point scoring in each playoffs series is binary; win or loss. The model was simplified to only compare the expected results to the random number, and any expected result greater than the random number was recorded as a win to Team A.
...
for (sim_no in 1:5000) {
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_3[nrow(First_Round_26_3) , "Won_B"] <- 1 - First_Round_26_3[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_3[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_3[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_3[nrow(First_Round_26_3)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_3[nrow(First_Round_26_3) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_3[nrow(First_Round_26_3) , "Won_B"] <- 1 - First_Round_26_3[ nrow(First_Round_26_3) , "Won_A"]
      
    }
    
    First_Round_26_3_Winner <- summarise(First_Round_26_3,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_3_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_3_Winner <- select(First_Round_26_3,
                                   contains(Winner_Suffix))
    
    First_Round_26_3_Winner <- First_Round_26_3_Winner[nrow(First_Round_26_3_Winner), ]
    
    colnames(First_Round_26_3_Winner) <- c("Team", "Rating", "Conference", "Won")
    
  }
  
  ## First Round 4
  First_Round_26_4 <- filter(ungroup(Finals),
                          Conference == "Eastern",
                          Division == "Metropolitan",
                          Position_in_Div == 2)
  
  First_Round_26_4 <- bind_rows(First_Round_26_4,
                             filter(ungroup(Finals),
                                    Conference == "Eastern",
                                    Division == "Metropolitan",
                                    Position_in_Div == 3))
  
  First_Round_26_4 <- transmute(First_Round_26_4,
                             Team_A = Team_Name[1],
                             Team_B = Team_Name[2],
                             Rating_A = Rating[1],
                             Rating_B = Rating[2],
  ...
  First_Round_26_4 <- mutate(First_Round_26_4,
                          Expected = (1 / 
                                        (1 + 10^(-(Rating_A - Rating_B) / 400))),
                          "Rand" = NA,
                          "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_4[nrow(First_Round_26_4) , "Rand"] <- runif(1, 0, 1)
      
      First_Round_26_4[nrow(First_Round_26_4) , "Won_A"] <- if_else(First_Round_26_4[nrow(First_Round_26_4) , "Rand"] <= 
                                                                First_Round_26_4[nrow(First_Round_26_4) , "Expected"],
                                                              true = 1,
                                                              false = 0)
      
      First_Round_26_4[nrow(First_Round_26_4) , "Won_B"] <- 1 - First_Round_26_4[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_4[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_4[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_4[nrow(First_Round_26_4)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_4[nrow(First_Round_26_4) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_4[nrow(First_Round_26_4) , "Won_B"] <- 1 - First_Round_26_4[ nrow(First_Round_26_4) , "Won_A"]
      
    }
    
    First_Round_26_4_Winner <- summarise(First_Round_26_4,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_4_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_4_Winner <- select(First_Round_26_4,
                                   contains(Winner_Suffix))
    
    First_Round_26_4_Winner <- First_Round_26_4_Winner[nrow(First_Round_26_4_Winner), ]
    
    colnames(First_Round_26_4_Winner) <- c("Team", "Rating", "Conference", "Won")
    
  }
  
  ## First Round 5
  First_Round_26_5 <- filter(ungroup(Finals),
                          Conference == "Western",
                          Division == "Central",
                          Position_in_Div == 1)
  
  First_Round_26_5 <- bind_rows(First_Round_26_5,
                             filter(ungroup(Finals),
                                    Conference == "Western",
                                    Wildcard_Rank == 1))
  
  First_Round_26_5 <- transmute(First_Round_26_5,
                             Team_A = Team_Name[1],
                             Team_B = Team_Name[2],
                             Rating_A = Rating[1],
                             Rating_B = Rating[2],
  ...
  First_Round_26_5 <- mutate(First_Round_26_5,
                          Expected = (1 / 
                                        (1 + 10^(-(Rating_A - Rating_B) / 400))),
                          "Rand" = NA,
                          "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_5[nrow(First_Round_26_5) , "Rand"] <- runif(1, 0, 1)
      
      First_Round_26_5[nrow(First_Round_26_5) , "Won_A"] <- if_else(First_Round_26_5[nrow(First_Round_26_5) , "Rand"] <= 
                                                                First_Round_26_5[nrow(First_Round_26_5) , "Expected"],
                                                              true = 1,
                                                              false = 0)
      
      First_Round_26_5[nrow(First_Round_26_5) , "Won_B"] <- 1 - First_Round_26_5[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_5[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_5[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_5[nrow(First_Round_26_5)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_5[nrow(First_Round_26_5) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_5[nrow(First_Round_26_5) , "Won_B"] <- 1 - First_Round_26_5[ nrow(First_Round_26_5) , "Won_A"]
      
    }
    
    First_Round_26_5_Winner <- summarise(First_Round_26_5,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_5_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_5_Winner <- select(First_Round_26_5,
                                   contains(Winner_Suffix))
    
    First_Round_26_5_Winner <- First_Round_26_5_Winner[nrow(First_Round_26_5_Winner), ]
    
    colnames(First_Round_26_5_Winner) <- c("Team", "Rating", "Conference", "Won")
    
  }
  
  ## First Round 6
  First_Round_26_6 <- filter(ungroup(Finals),
                          Conference == "Western",
                          Division == "Central",
                          Position_in_Div == 2)
  
  First_Round_26_6 <- bind_rows(First_Round_26_6,
                             filter(ungroup(Finals),
                                    Conference == "Western",
                                    Division == "Central",
                                    Position_in_Div == 3))
  
  First_Round_26_6 <- transmute(First_Round_26_6,
                             Team_A = Team_Name[1],
                             Team_B = Team_Name[2],
                             Rating_A = Rating[1],
                             Rating_B = Rating[2],
  ...
  First_Round_26_6 <- mutate(First_Round_26_6,
                          Expected = (1 / 
                                        (1 + 10^(-(Rating_A - Rating_B) / 400))),
                          "Rand" = NA,
                          "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_6[nrow(First_Round_26_6) , "Rand"] <- runif(1, 0, 1)
      
      First_Round_26_6[nrow(First_Round_26_6) , "Won_A"] <- if_else(First_Round_26_6[nrow(First_Round_26_6) , "Rand"] <= 
                                                                First_Round_26_6[nrow(First_Round_26_6) , "Expected"],
                                                              true = 1,
                                                              false = 0)
      
      First_Round_26_6[nrow(First_Round_26_6) , "Won_B"] <- 1 - First_Round_26_6[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_6[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_6[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_6[nrow(First_Round_26_6)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_6[nrow(First_Round_26_6) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_6[nrow(First_Round_26_6) , "Won_B"] <- 1 - First_Round_26_6[ nrow(First_Round_26_6) , "Won_A"]
      
    }
    
    First_Round_26_6_Winner <- summarise(First_Round_26_6,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_6_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_6_Winner <- select(First_Round_26_6,
                                   contains(Winner_Suffix))
    
    First_Round_26_6_Winner <- First_Round_26_6_Winner[nrow(First_Round_26_6_Winner), ]
    
    colnames(First_Round_26_6_Winner) <- c("Team", "Rating", "Conference", "Won")
    
  }
  
  ## First Round 7
  First_Round_26_7 <- filter(ungroup(Finals),
                          Conference == "Western",
                          Division == "Pacific",
                          Position_in_Div == 1)
  
  First_Round_26_7 <- bind_rows(First_Round_26_7,
                             filter(ungroup(Finals),
                                    Conference == "Western",
                                    Wildcard_Rank == 2))
  
  First_Round_26_7 <- transmute(First_Round_26_7,
                             Team_A = Team_Name[1],
                             Team_B = Team_Name[2],
                             Rating_A = Rating[1],
                             Rating_B = Rating[2],
  ...
  First_Round_26_7 <- mutate(First_Round_26_7,
                          Expected = (1 / 
                                        (1 + 10^(-(Rating_A - Rating_B) / 400))),
                          "Rand" = NA,
                          "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_7[nrow(First_Round_26_7) , "Rand"] <- runif(1, 0, 1)
      
      First_Round_26_7[nrow(First_Round_26_7) , "Won_A"] <- if_else(First_Round_26_7[nrow(First_Round_26_7) , "Rand"] <= 
                                                                First_Round_26_7[nrow(First_Round_26_7) , "Expected"],
                                                              true = 1,
                                                              false = 0)
      
      First_Round_26_7[nrow(First_Round_26_7) , "Won_B"] <- 1 - First_Round_26_7[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_7[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_7[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_7[nrow(First_Round_26_7)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_7[nrow(First_Round_26_7) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_7[nrow(First_Round_26_7) , "Won_B"] <- 1 - First_Round_26_7[ nrow(First_Round_26_7) , "Won_A"]
      
    }
    
    First_Round_26_7_Winner <- summarise(First_Round_26_7,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_7_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_7_Winner <- select(First_Round_26_7,
                                   contains(Winner_Suffix))
    
    First_Round_26_7_Winner <- First_Round_26_7_Winner[nrow(First_Round_26_7_Winner), ]
    
    colnames(First_Round_26_7_Winner) <- c("Team", "Rating", "Conference", "Won")
    
  }
  
  ## First Round 8
  First_Round_26_8 <- filter(ungroup(Finals),
                          Conference == "Western",
                          Division == "Pacific",
                          Position_in_Div == 2)
  
  First_Round_26_8 <- bind_rows(First_Round_26_8,
                             filter(ungroup(Finals),
                                    Conference == "Western",
                                    Division == "Pacific",
                                    Position_in_Div == 3))
  
  First_Round_26_8 <- transmute(First_Round_26_8,
                             Team_A = Team_Name[1],
                             Team_B = Team_Name[2],
                             Rating_A = Rating[1],
                             Rating_B = Rating[2],
  ...
  First_Round_26_8 <- mutate(First_Round_26_8,
                          Expected = (1 / 
                                        (1 + 10^(-(Rating_A - Rating_B) / 400))),
                          "Rand" = NA,
                          "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      First_Round_26_8[nrow(First_Round_26_8) , "Rand"] <- runif(1, 0, 1)
      
      First_Round_26_8[nrow(First_Round_26_8) , "Won_A"] <- if_else(First_Round_26_8[nrow(First_Round_26_8) , "Rand"] <= 
                                                                First_Round_26_8[nrow(First_Round_26_8) , "Expected"],
                                                              true = 1,
                                                              false = 0)
      
      First_Round_26_8[nrow(First_Round_26_8) , "Won_B"] <- 1 - First_Round_26_8[ , "Won_A"]
      
    }
    
    if (sum(First_Round_26_8[ , "Won_A"], na.rm = T) == 4 || sum(First_Round_26_8[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      First_Round_26_8[nrow(First_Round_26_8)+1 , c("Team_A",
                                              "Team_B",
                                              "Rating_A",
                                              "Rating_B",
                                              "Conference_A",
                                              "Conference_B",
                                              "Expected")] <- First_Round_26_8[nrow(First_Round_26_8) , c("Team_A",
                                                                                                    "Team_B",
                                                                                                    "Rating_A",
                                                                                                    "Rating_B",
                                                                                                    "Conference_A",
      ...
      First_Round_26_8[nrow(First_Round_26_8) , "Won_B"] <- 1 - First_Round_26_8[ nrow(First_Round_26_8) , "Won_A"]
      
    }
    
    First_Round_26_8_Winner <- summarise(First_Round_26_8,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(First_Round_26_8_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    First_Round_26_8_Winner <- select(First_Round_26_8,
                                   contains(Winner_Suffix))
    
    First_Round_26_8_Winner <- First_Round_26_8_Winner[nrow(First_Round_26_8_Winner), ]
    
    colnames(First_Round_26_8_Winner) <- c("Team", "Rating", "Conference", "Won")
  ...
  Second_Round_26_1 <- rbind(First_Round_26_1_Winner,
                          First_Round_26_2_Winner)
  
  Second_Round_26_1[ , "Won"] <- NA
  
  Second_Round_26_1 <- transmute(Second_Round_26_1,
                              Team_A = Team[1],
                              Team_B = Team[2],
                              Rating_A = Rating[1],
                              Rating_B = Rating[2],
  ...
  Second_Round_26_1 <- mutate(Second_Round_26_1,
                           Expected = (1 / 
                                         (1 + 10^(-(Rating_A - Rating_B) / 400))),
                           "Rand" = NA,
                           "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Second_Round_26_1[nrow(Second_Round_26_1) , "Rand"] <- runif(1, 0, 1)
      
      Second_Round_26_1[nrow(Second_Round_26_1) , "Won_A"] <- if_else(Second_Round_26_1[nrow(Second_Round_26_1) , "Rand"] <= 
                                                                  Second_Round_26_1[nrow(Second_Round_26_1) , "Expected"],
                                                                true = 1,
                                                                false = 0)
      
      Second_Round_26_1[nrow(Second_Round_26_1) , "Won_B"] <- 1 - Second_Round_26_1[ , "Won_A"]
      
    }
    
    if (sum(Second_Round_26_1[ , "Won_A"], na.rm = T) == 4 || sum(Second_Round_26_1[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Second_Round_26_1[nrow(Second_Round_26_1)+1 , c("Team_A",
                                                "Team_B",
                                                "Rating_A",
                                                "Rating_B",
                                                "Conference_A",
                                                "Conference_B",
                                                "Expected")] <- Second_Round_26_1[nrow(Second_Round_26_1) , c("Team_A",
                                                                                                        "Team_B",
                                                                                                        "Rating_A",
                                                                                                        "Rating_B",
                                                                                                        "Conference_A",
      ...
      Second_Round_26_1[nrow(Second_Round_26_1) , "Won_B"] <- 1 - Second_Round_26_1[ nrow(Second_Round_26_1) , "Won_A"]
      
    }
    
    Second_Round_26_1_Winner <- summarise(Second_Round_26_1,
                                       "Wins_A" = sum(Won_A),
                                       "Wins_B" = sum(Won_B),
                                       "Winner" = if_else(Wins_A == 4,
                                                          "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Second_Round_26_1_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Second_Round_26_1_Winner <- select(Second_Round_26_1,
                                    contains(Winner_Suffix))
    
    Second_Round_26_1_Winner <- Second_Round_26_1_Winner[nrow(Second_Round_26_1_Winner), ]
    
    colnames(Second_Round_26_1_Winner) <- c("Team", "Rating", "Conference", "Won")
  ...
  Second_Round_26_2 <- rbind(First_Round_26_3_Winner,
                          First_Round_26_4_Winner)
  
  Second_Round_26_2[ , "Won"] <- NA
  
  Second_Round_26_2 <- transmute(Second_Round_26_2,
                              Team_A = Team[1],
                              Team_B = Team[2],
                              Rating_A = Rating[1],
                              Rating_B = Rating[2],
  ...
  Second_Round_26_2 <- mutate(Second_Round_26_2,
                           Expected = (1 / 
                                         (1 + 10^(-(Rating_A - Rating_B) / 400))),
                           "Rand" = NA,
                           "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Second_Round_26_2[nrow(Second_Round_26_2) , "Rand"] <- runif(1, 0, 1)
      
      Second_Round_26_2[nrow(Second_Round_26_2) , "Won_A"] <- if_else(Second_Round_26_2[nrow(Second_Round_26_2) , "Rand"] <= 
                                                                  Second_Round_26_2[nrow(Second_Round_26_2) , "Expected"],
                                                                true = 1,
                                                                false = 0)
      
      Second_Round_26_2[nrow(Second_Round_26_2) , "Won_B"] <- 1 - Second_Round_26_2[ , "Won_A"]
      
    }
    
    if (sum(Second_Round_26_2[ , "Won_A"], na.rm = T) == 4 || sum(Second_Round_26_2[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Second_Round_26_2[nrow(Second_Round_26_2)+1 , c("Team_A",
                                                "Team_B",
                                                "Rating_A",
                                                "Rating_B",
                                                "Conference_A",
                                                "Conference_B",
                                                "Expected")] <- Second_Round_26_2[nrow(Second_Round_26_2) , c("Team_A",
                                                                                                        "Team_B",
                                                                                                        "Rating_A",
                                                                                                        "Rating_B",
                                                                                                        "Conference_A",
      ...
      Second_Round_26_2[nrow(Second_Round_26_2) , "Won_B"] <- 1 - Second_Round_26_2[ nrow(Second_Round_26_2) , "Won_A"]
      
    }
    
    Second_Round_26_2_Winner <- summarise(Second_Round_26_2,
                                       "Wins_A" = sum(Won_A),
                                       "Wins_B" = sum(Won_B),
                                       "Winner" = if_else(Wins_A == 4,
                                                          "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Second_Round_26_2_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Second_Round_26_2_Winner <- select(Second_Round_26_2,
                                    contains(Winner_Suffix))
    
    Second_Round_26_2_Winner <- Second_Round_26_2_Winner[nrow(Second_Round_26_2_Winner), ]
    
    colnames(Second_Round_26_2_Winner) <- c("Team", "Rating", "Conference", "Won")
  }
  
  ## Second_Round_26_3
  
  Second_Round_26_3 <- rbind(First_Round_26_5_Winner,
                          First_Round_26_6_Winner)
  
  Second_Round_26_3[ , "Won"] <- NA
  
  Second_Round_26_3 <- transmute(Second_Round_26_3,
                              Team_A = Team[1],
                              Team_B = Team[2],
                              Rating_A = Rating[1],
                              Rating_B = Rating[2],
  ...
  Second_Round_26_3 <- mutate(Second_Round_26_3,
                           Expected = (1 / 
                                         (1 + 10^(-(Rating_A - Rating_B) / 400))),
                           "Rand" = NA,
                           "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Second_Round_26_3[nrow(Second_Round_26_3) , "Rand"] <- runif(1, 0, 1)
      
      Second_Round_26_3[nrow(Second_Round_26_3) , "Won_A"] <- if_else(Second_Round_26_3[nrow(Second_Round_26_3) , "Rand"] <= 
                                                                  Second_Round_26_3[nrow(Second_Round_26_3) , "Expected"],
                                                                true = 1,
                                                                false = 0)
      
      Second_Round_26_3[nrow(Second_Round_26_3) , "Won_B"] <- 1 - Second_Round_26_3[ , "Won_A"]
      
    }
    
    if (sum(Second_Round_26_3[ , "Won_A"], na.rm = T) == 4 || sum(Second_Round_26_3[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Second_Round_26_3[nrow(Second_Round_26_3)+1 , c("Team_A",
                                                "Team_B",
                                                "Rating_A",
                                                "Rating_B",
                                                "Conference_A",
                                                "Conference_B",
                                                "Expected")] <- Second_Round_26_3[nrow(Second_Round_26_3) , c("Team_A",
                                                                                                        "Team_B",
                                                                                                        "Rating_A",
                                                                                                        "Rating_B",
                                                                                                        "Conference_A",
      ...
      Second_Round_26_3[nrow(Second_Round_26_3) , "Won_B"] <- 1 - Second_Round_26_3[ nrow(Second_Round_26_3) , "Won_A"]
      
    }
    
    Second_Round_26_3_Winner <- summarise(Second_Round_26_3,
                                       "Wins_A" = sum(Won_A),
                                       "Wins_B" = sum(Won_B),
                                       "Winner" = if_else(Wins_A == 4,
                                                          "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Second_Round_26_3_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Second_Round_26_3_Winner <- select(Second_Round_26_3,
                                    contains(Winner_Suffix))
    
    Second_Round_26_3_Winner <- Second_Round_26_3_Winner[nrow(Second_Round_26_3_Winner), ]
    
    colnames(Second_Round_26_3_Winner) <- c("Team", "Rating", "Conference", "Won")
  }
  
  ## Second_Round_26_4
  
  Second_Round_26_4 <- rbind(First_Round_26_7_Winner,
                          First_Round_26_8_Winner)
  
  Second_Round_26_4[ , "Won"] <- NA
  
  Second_Round_26_4 <- transmute(Second_Round_26_4,
                              Team_A = Team[1],
                              Team_B = Team[2],
                              Rating_A = Rating[1],
                              Rating_B = Rating[2],
  ...
  Second_Round_26_4 <- mutate(Second_Round_26_4,
                           Expected = (1 / 
                                         (1 + 10^(-(Rating_A - Rating_B) / 400))),
                           "Rand" = NA,
                           "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Second_Round_26_4[nrow(Second_Round_26_4) , "Rand"] <- runif(1, 0, 1)
      
      Second_Round_26_4[nrow(Second_Round_26_4) , "Won_A"] <- if_else(Second_Round_26_4[nrow(Second_Round_26_4) , "Rand"] <= 
                                                                  Second_Round_26_4[nrow(Second_Round_26_4) , "Expected"],
                                                                true = 1,
                                                                false = 0)
      
      Second_Round_26_4[nrow(Second_Round_26_4) , "Won_B"] <- 1 - Second_Round_26_4[ , "Won_A"]
      
    }
    
    if (sum(Second_Round_26_4[ , "Won_A"], na.rm = T) == 4 || sum(Second_Round_26_4[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Second_Round_26_4[nrow(Second_Round_26_4)+1 , c("Team_A",
                                                "Team_B",
                                                "Rating_A",
                                                "Rating_B",
                                                "Conference_A",
                                                "Conference_B",
                                                "Expected")] <- Second_Round_26_4[nrow(Second_Round_26_4) , c("Team_A",
                                                                                                        "Team_B",
                                                                                                        "Rating_A",
                                                                                                        "Rating_B",
                                                                                                        "Conference_A",
      ...
      Second_Round_26_4[nrow(Second_Round_26_4) , "Won_B"] <- 1 - Second_Round_26_4[ nrow(Second_Round_26_4) , "Won_A"]
      
    }
    
    Second_Round_26_4_Winner <- summarise(Second_Round_26_4,
                                       "Wins_A" = sum(Won_A),
                                       "Wins_B" = sum(Won_B),
                                       "Winner" = if_else(Wins_A == 4,
                                                          "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Second_Round_26_4_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Second_Round_26_4_Winner <- select(Second_Round_26_4,
                                    contains(Winner_Suffix))
    
    Second_Round_26_4_Winner <- Second_Round_26_4_Winner[nrow(Second_Round_26_4_Winner), ]
    
    colnames(Second_Round_26_4_Winner) <- c("Team", "Rating", "Conference", "Won")
  }
  ...
  Conf_Round_26_1 <- rbind(Second_Round_26_1_Winner,
                        Second_Round_26_2_Winner)
  
  Conf_Round_26_1[ , "Won"] <- NA
  
  Conf_Round_26_1 <- transmute(Conf_Round_26_1,
                              Team_A = Team[1],
                              Team_B = Team[2],
                              Rating_A = Rating[1],
                              Rating_B = Rating[2],
  ...
  Conf_Round_26_1 <- mutate(Conf_Round_26_1,
                           Expected = (1 / 
                                         (1 + 10^(-(Rating_A - Rating_B) / 400))),
                           "Rand" = NA,
                           "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Conf_Round_26_1[nrow(Conf_Round_26_1) , "Rand"] <- runif(1, 0, 1)
      
      Conf_Round_26_1[nrow(Conf_Round_26_1) , "Won_A"] <- if_else(Conf_Round_26_1[nrow(Conf_Round_26_1) , "Rand"] <= 
                                                                  Conf_Round_26_1[nrow(Conf_Round_26_1) , "Expected"],
                                                                true = 1,
                                                                false = 0)
      
      Conf_Round_26_1[nrow(Conf_Round_26_1) , "Won_B"] <- 1 - Conf_Round_26_1[ , "Won_A"]
      
    }
    
    if (sum(Conf_Round_26_1[ , "Won_A"], na.rm = T) == 4 || sum(Conf_Round_26_1[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Conf_Round_26_1[nrow(Conf_Round_26_1)+1 , c("Team_A",
                                                "Team_B",
                                                "Rating_A",
                                                "Rating_B",
                                                "Conference_A",
                                                "Conference_B",
                                                "Expected")] <- Conf_Round_26_1[nrow(Conf_Round_26_1) , c("Team_A",
                                                                                                        "Team_B",
                                                                                                        "Rating_A",
                                                                                                        "Rating_B",
                                                                                                        "Conference_A",
      ...
      Conf_Round_26_1[nrow(Conf_Round_26_1) , "Won_B"] <- 1 - Conf_Round_26_1[ nrow(Conf_Round_26_1) , "Won_A"]
      
    }
    
    Conf_Round_26_1_Winner <- summarise(Conf_Round_26_1,
                                       "Wins_A" = sum(Won_A),
                                       "Wins_B" = sum(Won_B),
                                       "Winner" = if_else(Wins_A == 4,
                                                          "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Conf_Round_26_1_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Conf_Round_26_1_Winner <- select(Conf_Round_26_1,
                                    contains(Winner_Suffix))
    
    Conf_Round_26_1_Winner <- Conf_Round_26_1_Winner[nrow(Conf_Round_26_1_Winner), ]
    
    colnames(Conf_Round_26_1_Winner) <- c("Team", "Rating", "Conference", "Won")
  }
  
  ## Conference_Round_2
  
  Conf_Round_26_2 <- rbind(Second_Round_26_3_Winner,
                        Second_Round_26_4_Winner)
  
  Conf_Round_26_2[ , "Won"] <- NA
  
  Conf_Round_26_2 <- transmute(Conf_Round_26_2,
                            Team_A = Team[1],
                            Team_B = Team[2],
                            Rating_A = Rating[1],
                            Rating_B = Rating[2],
  ...
  Conf_Round_26_2 <- mutate(Conf_Round_26_2,
                         Expected = (1 / 
                                       (1 + 10^(-(Rating_A - Rating_B) / 400))),
                         "Rand" = NA,
                         "Won_A" = NA,
  ...
  for (i in 1:7) {
    ...
    if (i == 1) {
      ...
      Conf_Round_26_2[nrow(Conf_Round_26_2) , "Rand"] <- runif(1, 0, 1)
      
      Conf_Round_26_2[nrow(Conf_Round_26_2) , "Won_A"] <- if_else(Conf_Round_26_2[nrow(Conf_Round_26_2) , "Rand"] <= 
                                                              Conf_Round_26_2[nrow(Conf_Round_26_2) , "Expected"],
                                                            true = 1,
                                                            false = 0)
      
      Conf_Round_26_2[nrow(Conf_Round_26_2) , "Won_B"] <- 1 - Conf_Round_26_2[ , "Won_A"]
      
    }
    
    if (sum(Conf_Round_26_2[ , "Won_A"], na.rm = T) == 4 || sum(Conf_Round_26_2[ , "Won_B"], na.rm = T) == 4) {
      
      break
      
    } else {
      
      Conf_Round_26_2[nrow(Conf_Round_26_2)+1 , c("Team_A",
                                            "Team_B",
                                            "Rating_A",
                                            "Rating_B",
                                            "Conference_A",
                                            "Conference_B",
                                            "Expected")] <- Conf_Round_26_2[nrow(Conf_Round_26_2) , c("Team_A",
                                                                                                "Team_B",
                                                                                                "Rating_A",
                                                                                                "Rating_B",
                                                                                                "Conference_A",
      ...
      Conf_Round_26_2[nrow(Conf_Round_26_2) , "Won_B"] <- 1 - Conf_Round_26_2[ nrow(Conf_Round_26_2) , "Won_A"]
      
    }
    
    Conf_Round_26_2_Winner <- summarise(Conf_Round_26_2,
                                     "Wins_A" = sum(Won_A),
                                     "Wins_B" = sum(Won_B),
                                     "Winner" = if_else(Wins_A == 4,
                                                        "Team_A", "Team_B"))
    
    Winner_Suffix <- if_else(str_detect(Conf_Round_26_2_Winner$Winner,
                                        "Team_A"),
                             true = "_A", false = "_B")
    
    Conf_Round_26_2_Winner <- select(Conf_Round_26_2,
                                  contains(Winner_Suffix))
    
    Conf_Round_26_2_Winner <- Conf_Round_26_2_Winner[nrow(Conf_Round_26_2_Winner), ]
    
    colnames(Conf_Round_26_2_Winner) <- c("Team", "Rating", "Conference", "Won")
  }
  
  ## Conference_Round_1
  
  Stanley_Cup_Round <- rbind(Conf_Round_26_1_Winner,
                             Conf_Round_26_2_Winner)
  ...
  for (round_ind in ls()[which(str_detect(ls(), "Second_Round_26_\\d$"))] ) {
  ...
  for (round_ind in ls()[which(str_detect(ls(), "Conf_Round_26_\\d$"))] ) {
...
ggplot(filter(Monte_Results_26,
              !str_detect(Team_Name,
                          "Louis")),
       aes(x = Position)) +
  ...
  labs(caption = "Note changes in y-axes") +
  theme_minimal()
...
St_Louis_Results_26 <- filter(summarise(group_by(Monte_Results_26,
                                              Team_Name, Position),
                                     "Count" = n()),
                           Team_Name == "St. Louis Blues")

ggplot(St_Louis_Results_26,
       aes(x = Position, y = Count)) +
  geom_bar(stat = "identity",
           col = "yellow",
           fill = "blue3") +
  ggtitle(label = paste("Frequencies of Season Finishing Position for St. Louis Blues Across",
                        comma(sim_no), "Trials"),
          subtitle = "Using the Elo 25 Partition") +
...
for (sim_no in 1:5000) {
  ...
  for (i in 1:7) {
    ...
    first_round_26_1_winner <- summarise(First_Round_1,
                                      "Wins_A" = sum(Won_A),
                                      "Wins_B" = sum(Won_B),
                                      "Winner" = if_else(Wins_A == 4,
                                                         "Team_A", "Team_B"))
  ...
  colnames(Monte_Results_Stanley_Round_Winner_51) <- c("Team", "Rating", "Conference", "Won", "Sim_No")
...
ggplot(filter(Monte_Results_Season_51,
              !str_detect(Team_Name,
                          "Louis")),
       aes(x = Position)) +
  geom_histogram(fill = "cyan2",
...
for (sim_no in 1:5000) {
  ...
  colnames(Monte_Results_Stanley_Round_Winner_82) <- c("Team", "Rating", "Conference", "Won", "Sim_No")
...
St_Louis_Season_26 <- filter(Monte_Results_26,
                             str_detect(Team_Name, "Louis"))

St_Louis_26_Median <- median(St_Louis_Season_26$Position)

St_Louis_26_Mean <- mean(St_Louis_Season_26$Position)


St_Louis_Season_51 <- filter(Monte_Results_Season_51,
                             str_detect(Team_Name, "Louis"))

St_Louis_51_Median <- median(St_Louis_Season_51$Position)

St_Louis_51_Mean <- mean(St_Louis_Season_51$Position)


```

As can be seen, the St. Louis Blues' distribution of simulated finishing posisitions changed substantially between the two data partitions at game 25 and game 50. The distribution of finishing positions using partition 25 was left-skewed and with a median of `r St_Louis_26_Median` and a mean of `r St_Louis_26_Mean`. The distribution of simulated season finishing positions shifted with the game 50 partition to be centred around a median of `r St_Louis_51_Median` and a mean of `r St_Louis_51_Mean`. Unlike the game 25 partition, the game 50 partition yielded a more symmetrical distribution of finishing positions.

# Discussion

```{r message=FALSE, warning=FALSE, include=FALSE}

St_Louis_Cup_Wins_26 <- nrow(filter(Monte_Results_Stanley_Round_Winner_26,
                                    str_detect(Team, "Louis")))

St_Louis_Cup_Wins_51 <- nrow(filter(Monte_Results_Stanley_Round_Winner_51,
                                    str_detect(Team, "Louis")))

St_Louis_Cup_Wins_82 <- nrow(filter(Monte_Results_Stanley_Round_Winner_82,
                                    str_detect(Team, "Louis")))
...
The simulations of the three partitions reflected the marked improvement in the St. Louis Blues' performance throughout the season. Using the game 25 partition data, the St. Louis Blues won the Stanley Cup a total of `r comma(St_Louis_Cup_Wins_26)` times out of `r comma(sim_no)` simulations, which was a likelihood of `r percent(St_Louis_Cup_Wins_26 / sim_no, accuracy = 0.01)`.  This improved somewhat when comparing to the game 50 partition data, which saw the St Louis Blues win a total of `r comma(St_Louis_Cup_Wins_51)` out of `r comma(sim_no)` simulations (likelihood of `r percent(St_Louis_Cup_Wins_51 / sim_no, accuracy = 0.01)`). The biggest improvement was seen when simulating the playoffs series based on their standings and performance at the end of the 82-game regular season. Using the 82-game non-partitioned data, the St Louis Blues won the Stanley Cup a total of `r comma(St_Louis_Cup_Wins_82)` out of a total `r comma(sim_no)` trials, which was a likelihood of   `r percent(St_Louis_Cup_Wins_82 / sim_no, accuracy = 0.01)`.

This improvement throughout the season was as anticipated, otherwise the Blues would not have been able to win the Stanley Cup. What was most interesting, however, was that the St. Louis Blues still managed to win the Stanley Cup despite the simulations using the non-partitioned season data showing their likelihood of winning to be a mere `r percent(St_Louis_Cup_Wins_82 / sim_no, accuracy = 0.01)`

## Other Teams - Noteworthy Results

```{r include=FALSE, message=FALSE, warning=FALSE}

## Tampa Bay

Boston_Cup_Wins_26 <- nrow(filter(Monte_Results_Stanley_Round_Winner_26,
                                    str_detect(Team, "Boston")))

Boston_Cup_Wins_51 <- nrow(filter(Monte_Results_Stanley_Round_Winner_51,
                                    str_detect(Team, "Boston")))

Boston_Cup_Wins_82 <- nrow(filter(Monte_Results_Stanley_Round_Winner_82,
                                    str_detect(Team, "Boston")))

## Tampa Bay

Tampa_Cup_Wins_26 <- nrow(filter(Monte_Results_Stanley_Round_Winner_26,
                                    str_detect(Team, "Tampa")))

Tampa_Cup_Wins_51 <- nrow(filter(Monte_Results_Stanley_Round_Winner_51,
                                    str_detect(Team, "Tampa")))

Tampa_Cup_Wins_82 <- nrow(filter(Monte_Results_Stanley_Round_Winner_82,
                                    str_detect(Team, "Tampa")))


## Nashville

Nashville_Cup_Wins_26 <- nrow(filter(Monte_Results_Stanley_Round_Winner_26,
                                     str_detect(Team, "Nashville")))

Nashville_Cup_Wins_51 <- nrow(filter(Monte_Results_Stanley_Round_Winner_51,
                                    str_detect(Team, "Nashville")))

Nashville_Cup_Wins_82 <- nrow(filter(Monte_Results_Stanley_Round_Winner_82,
                                    str_detect(Team, "Nashville")))
...
Despite their long and repeated history of Stanley Cup playoffs appearances, the Boston Bruins were no more a certainty of making the finals than most other teams. After their 25th game, the simulations showed Boston won the Cup `r comma(Boston_Cup_Wins_26)` out of `r comma(sim_no)` simulations, which was a likelihood of `r percent(Boston_Cup_Wins_26 / sim_no, accuracy = 0.01)`. This changed a little, but not much in the general context of the competition, when using the 50 game partition. In these simulations Boston took home the Stanley Cup a total of `r comma(Boston_Cup_Wins_51)` times from `r comma(sim_no)`, which was a likelihood of `r percent(Boston_Cup_Wins_51 / sim_no, accuracy = 0.01)`. Of the simulations of non-partitioned data, where the Bruins' place in the playoffs was clinched, they won the Stanley Cup `r comma(Boston_Cup_Wins_82)` out of `r comma(sim_no)` simulations, a likelihood of `r percent(Boston_Cup_Wins_82 / sim_no, accuracy = 0.01)`. Note that this is a slightly greater likelihood of winning the Cup than the St. Louis Blues (`r percent(St_Louis_Cup_Wins_82 / sim_no, accuracy = 0.01)`).

### Tampa Bay

It could be argued that the Tampa Bay Lightning were robbed of the Stanley Cup this season. In the real data for the season, Tampa Bay performed consistently throughout all fixtures and finished with the most season points, earning them the Presidents' Trophy. In the simulations for the game 25 and game 50 partitions, Tampa Bay won the Cup a total of `r comma(Tampa_Cup_Wins_26)` and `r comma(Tampa_Cup_Wins_51)` times, respectively. This equated to a likelihood of `r percent(Tampa_Cup_Wins_26 / sim_no, accuracy = 0.01)` and `r percent(Tampa_Cup_Wins_51 / sim_no, accuracy = 0.01)`. This did not change by the end of the season, and the playoffs using the non-partitioned 82 game data saw Tampa Bay winning a total of `r comma(Tampa_Cup_Wins_82)` Stanley Cups, which was a likelihood of `r percent(Tampa_Cup_Wins_82 / sim_no, accuracy = 0.01)`.

### Nashville

Nashville had a very strong start to the season, as shown in the results from the simulations using the 25 game partition. Nashville won a total of `r comma(Nashville_Cup_Wins_26)` times out of `r comma(sim_no)` simulations, giving a likelihood of `r percent(Nashville_Cup_Wins_26 / sim_no, accuracy = 0.01)`. This likelihood of winning the Stanley Cup at game 25 was greater than even Tampa Bay. This high performance for Nashville waned as the season progressed. The 50 game partition saw the Nashville Predators win the Stanley Cup a total of `r comma(Nashville_Cup_Wins_51)` times out of `r comma(sim_no)` simulations, which was a likelihood of just `r percent(Nashville_Cup_Wins_51 / sim_no, accuracy = 0.01)`. The Predator's chances of winning the Stanley Cup increased slightly when using their performance across the whole season in the 82-game non-partition. Here Nashville took home the Cup a total of `r comma(Nashville_Cup_Wins_82)` times from `r comma(sim_no)`, which was a likelihood of `r percent(Nashville_Cup_Wins_82 / sim_no, accuracy = 0.01)`.

## Limitations

### Partitioning

Due to the staggered nature of number of games played across teams, some data was lost when partitioning on game number. For example, if all fixtures from game 25 onwards were filtered out of a data partition, games where Team A were playing their 23rd game and Team B were playing their 26th game were lost; neither considered in the constant Elo rating optimisation nor the partitioned data used to simulate the remaining season. A more effective method for partitioning the data would be based on date coupled with a refining of the season standings scoring method.

### Season Standings

The method used for calculating the season standings was too simple for the intended purpose. Points were calculated simply based on whether the team won, lost, or had an overtime loss. This greatly contrasts the season standings scoring method used by the NHL, which is more complex and comprises goals scored differential, games results between tied teams, and season points in the context of number of games played.

### Simulated Game Structure

The simulations could have also been improved by incorporating a simulation of points scored, which would have assisted with determining season standings (as outlined above) as well as provided a more realistic simulation of the regular season. Overtime losses were based on the difference between a random number and Elo expected results, which reduced the results of the game to a ternary data structure (0, 1, 2). Using a multiple regression model to simulate goals scored by teams would facilitate adding this level of granularity to the simulations.

### Replicating the Stanley Cup Playoffs

Whilst the St. Louis Blues and Boston Bruins were guaranteed spots in the playoffs based on their standings after the regular season ended, the simulations did not deliberately cover this scenario. Granted, the St. Louis Blues and Boston Bruins did meet in the Stanley Cup in the simulations, but they did so purely by the chance of the random number generator. It would be of interest to create further simulations specifically for the Stanley Cup Finals that replicated the team most likely to win between the St. Louis Blues and the Boston Bruins.

### Elo Ratings - Recency

Part of the reason why the St. Louis Blues winning the Stanley Cup was so sensational was that they stormed home at the end of the season from rocky beginnings. The Elo ratings used for the simulations based on the non-partitioned 82-game season would have failed to adequately measure the strength of the St. Louis Blues in the finals 30 or so games of the regular season. As such, the Blues may have been underrepresented in these simulations. Creating optimised Elo ratings based on a more recent and similarly sized partition of the data, for example between games 50 and 82, would allow for simulations to be run that reflect the strong performance of the St. Louis Blues in the end of the regular season.

# Conclusions

This study made some interesting findings, even if they were slightly as anticipated. The change in the St. Louis Blues' performance was well reflected in the simulations. The simulations also revealed that, even after the St. Louis Blues' chances of making the finals from their standings at the end of their 25th game were approaching nil, the large turn around in their season that was required to make the playoffs still meant that their likelihood of winning the Stanley Cup was low compared to their competitors.

The Boston Bruins were shown to have similar, but slightly greater, likelihood of winning the Stanley Cup than the St. Louis Blues when using Elo ratings from the entire season. At the two other partitions considered, at games 25 and 50, the Boston Bruins had a substantially greater likelihood of winning the Stanley Cup. Further simulations were justified that specifically simulated the playoffs between the Boston Bruins and the St. Louis Blues.
...
Sports Reference LLC., 2019, *2018-19 NHL Schedule and Results*, Hockey-Reference.com - Hockey Statistics and History, viewed 22 May 2019,  <https://www.hockey-reference.com/leagues/NHL_2019_games.html>
