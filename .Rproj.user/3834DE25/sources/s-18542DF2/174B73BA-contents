...
ggplot(Monte_Results_Season_82,
       aes(x = Points)) +
...
St_Louis_Results <- filter(summarise(group_by(Monte_Results_Season_82,
                                              Team_Name, Position),
                                     "Count" = n()),
...
ggplot(St_Louis_Results,
  ...
  geom_bar(stat = "identity",
           ...
           fill = "blue3",
           size = 1.05) +
  ggtitle(label = "Frequencies of Finishing Position for St. Louis Blues",
          subtitle = paste("From", comma(sim_no), "Trials")) +
...
colnames(Monte_Results_Stanley_Round_Winner_82) <- c("Team", 
                                                     "Rating",
                                                     "Conference", 
                                                     "Won", 
                                                     "Sim_No")

Stanley_Cup_Frequencies <- summarise(group_by(Monte_Results_Stanley_Round_Winner_82,
                                              Team),
                                     No_Cups = sum(Won),
                                     Likelihood = No_Cups / sum(ungroup(Monte_Results_Stanley_Round_Winner_82)$Won))
