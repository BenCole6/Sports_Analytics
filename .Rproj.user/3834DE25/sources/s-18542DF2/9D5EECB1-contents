...
ggplot(Monte_Results_Season_51,
       aes(x = Points)) +
...
St_Louis_Results <- filter(summarise(group_by(Monte_Results_Season_51,
                                              Team_Name, Position),
                                     "Count" = n()),
...
p_Elo50_Stanley_Winner <- ggplot(Monte_Results_Stanley_Round_Winner_51) +
  geom_bar(aes(x = Team),
           fill = "green4",
           alpha = 0.75) +
  scale_x_discrete("Team",
                   limits = rev(levels(as.factor(Monte_Results_Stanley_Round_Winner_51$Team)))) +
...
colnames(Monte_Results_Stanley_Round_Winner_51) <- c("Team", 
                                                  "Rating",
                                                  "Conference", 
                                                  "Won", 
                                                  "Sim_No")

Stanley_Cup_Frequencies <- summarise(group_by(Monte_Results_Stanley_Round_Winner_51,
                                              Team),
                                     No_Cups = sum(Won),
                                     Likelihood = No_Cups / sum(ungroup(Monte_Results_Stanley_Round_Winner_51)$Won))



ggplot(Stanley_Cup_Frequencies) +
  geom_bar(aes(x = Team,
               y = No_Cups),
           stat = "identity") +
  theme_minimal()
